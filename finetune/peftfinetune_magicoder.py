...
def train():
    ...
    if training_args.split == 0:
        raw_train_datasets = load_dataset(
            "ise-uiuc/Magicoder-OSS-Instruct-75K",
            split="train"
        )
    else:
        ratio = 100 // training_args.num_splits
        split = f"train[{ratio*(training_args.split-1)}%:{ratio*training_args.split}%]"
        print(f"Load data from {split}")
        raw_train_datasets = load_dataset(
            "ise-uiuc/Magicoder-OSS-Instruct-75K",
            split=split,
        )
