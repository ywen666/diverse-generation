...
class BaseInfluenceModule(abc.ABC):
    ...
    def __init__(
        ...
        self.cache_file_name = cache_name
    ...
    def influences(
        ...
        with tqdm(total=len(train_idxs)) as pbar:
            for grad_z, _ in self._loss_grad_loader_wrapper(batch_size=1, subset=train_idxs, train=True):
                ...
                scores.append(s)
                iteration_times += 1
                if (iteration_times+1) % 100 == 0:
                    json.dump(
                        (torch.tensor(scores) / len(self.train_loader.dataset)).tolist(),
                        pathlib.Path(self.cache_file_name).open('w'),
                        indent=2
                    )
                pbar.update(1)
